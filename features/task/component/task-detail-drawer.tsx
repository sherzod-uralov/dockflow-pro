...
const SubtaskItem = ({
  ...
  const statusOption = TASK_STATUS_OPTIONS.find((s) => s.value === subtask.status);
  const priorityOption = TASK_PRIORITY_OPTIONS.find((p) => p.value === subtask.priority);
...
export const TaskDetailDrawer = ({
  ...
  // Subtasks - tasks where parentTaskId equals current task
  ...
  const isWatching = watchers.length > 0; // Simplified check
  ...
  // Handle subtask click - open in same drawer
  ...
  return (
    <>
      <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <SheetContent
          side="right"
          className={`p-0 ${isFullscreen ? "w-full sm:max-w-full" : "w-full sm:max-w-2xl"} overflow-hidden [&>button]:hidden`}
        >
        {taskLoading ? (
              ...
              <Tabs.List px="md" style={{ flexShrink: 0 }}>
                ...
                <Tabs.Tab
                  ...
                  rightSection={
                    subtasks.length > 0 ? (
                      <Badge size="xs" variant="light" color="blue">
                        {subtasks.filter((s) => s.status === TaskStatus.COMPLETED).length}/{subtasks.length}
                      </Badge>
                    ) : null
                  }
                >
                                ...
                                {allUsers.map((user) => {
                                  ...
                                  return (
                                    <Paper
                                      ...
                                      radius="sm"
                                      style={{
                                        backgroundColor: isAssigned ? "#e7f5ff" : "#f8f9fa",
                                        cursor: "pointer",
                                      }}
                                      onClick={() => {
                      ...
                      {subtasks.length > 0 && (
                        ...
                        >
                          <Box
                            style={{
                              height: "100%",
                              width: `${(subtasks.filter((s) => s.status === TaskStatus.COMPLETED).length / subtasks.length) * 100}%`,
                              backgroundColor: subtasks.filter((s) => s.status === TaskStatus.COMPLETED).length === subtasks.length ? "#2ecc71" : "#1e3a5f",
                              borderRadius: 3,
                        ...
                        {subtasks.map((subtask) => (
                          ...
                          >
                            <Group justify="space-between" wrap="nowrap">
                              <Group gap="sm" wrap="nowrap" style={{ flex: 1, minWidth: 0 }}>
                                <ActionIcon
                                  variant={subtask.status === TaskStatus.COMPLETED ? "filled" : "outline"}
                                  size="xs"
                                  color={subtask.status === TaskStatus.COMPLETED ? "green" : "gray"}
                                  radius="xl"
                                  onClick={() =>
                                ...
                                >
                                  {subtask.status === TaskStatus.COMPLETED && <IconCheck size={10} />}
                                </ActionIcon>
                                <Text
                                  size="sm"
                                  c={subtask.status === TaskStatus.COMPLETED ? "dimmed" : "#212529"}
                                  lineClamp={1}
                                  style={{
                                    textDecoration: subtask.status === TaskStatus.COMPLETED ? "line-through" : "none",
                                    flex: 1,
                                    cursor: "pointer",
                                  }}
